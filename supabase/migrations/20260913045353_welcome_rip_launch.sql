create table if not exists public.welcome_rip_prizes (
  ...
  card_name text, card_set text, card_number text, image_url text,
...
create table if not exists public.welcome_rip_claims (
  ...
  prize_id uuid not null unique references public.welcome_rip_prizes(id) on delete restrict, claimed_at timestamptz not null default now()
);
create table if not exists public.welcome_rip_settings (
  singleton boolean primary key default true check (singleton = true), launch_at timestamptz not null default now(),
...
alter table public.welcome_rip_prizes enable row level security; alter table public.welcome_rip_claims enable row level security; alter table public.welcome_rip_settings enable row level security;
revoke all on public.welcome_rip_prizes from anon,authenticated; revoke all on public.welcome_rip_claims from anon,authenticated; revoke all on public.welcome_rip_settings from anon,authenticated;
grant select,insert,update,delete on public.welcome_rip_prizes to service_role; grant select,insert,update,delete on public.welcome_rip_claims to service_role; grant select,insert,update,delete on public.welcome_rip_settings to service_role;
...
begin
 ...
 select u.created_at,u.email_confirmed_at into account_created,account_confirmed from auth.users u where u.id=target_user_id;
...
revoke all on function public.claim_welcome_rip(uuid) from public,anon,authenticated; grant execute on function public.claim_welcome_rip(uuid) to service_role;
