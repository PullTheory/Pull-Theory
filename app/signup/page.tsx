...
export default function SignupPage() {
  ...
  async function handleSignup(event: FormEvent<HTMLFormElement>) {
    ...
    const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { username: normalizedUsername, membership_plan: plan, terms_accepted: true, terms_accepted_at: new Date().toISOString(), terms_version: "2026-08-30" } } });
    if (error) setMessage(error.message);
    else if (data?.session?.user) router.replace(returnTo);
    else { setConfirmationEmail(email.trim()); void recordTrafficEvent("confirmation_sent"); }
    setLoading(false);
  }
  ...
  return <main className="min-h-screen bg-black px-6 py-12 text-white">
    <div className="mx-auto max-w-md rounded-3xl border border-violet-500/30 bg-white/5 p-8 shadow-2xl shadow-violet-950/40 backdrop-blur">
      ...
      <form onSubmit={handleSignup} className="mt-8 space-y-5">
        ...
        <div><label htmlFor="password" className="mb-2 block text-sm text-zinc-300">Password</label><input id="password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-white outline-none focus:border-violet-400" placeholder="Your password"/></div>
        {message && <p className="rounded-xl bg-violet-500/10 p-3 text-sm text-violet-200">{message}</p>}
    ...
    {confirmationEmail && <div aria-labelledby="confirm-email-title" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-6 py-8 backdrop-blur-sm" role="dialog"><div className="w-full max-w-md rounded-3xl border border-violet-400/40 bg-zinc-950 p-7 text-center shadow-2xl"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-emerald-300/40 bg-emerald-400/10 text-2xl text-emerald-200">✓</div><p className="mt-5 text-xs font-bold uppercase tracking-[0.22em] text-violet-300">One more step</p><h2 id="confirm-email-title" className="mt-2 text-3xl font-semibold">Confirm your email</h2><p className="mt-4 text-sm leading-6 text-zinc-300">We sent a confirmation link to <span className="font-semibold text-white">{confirmationEmail}</span>.</p><div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-4 text-left text-sm leading-6 text-zinc-300"><p><span className="font-semibold text-white">1.</span> Open your email inbox.</p><p><span className="font-semibold text-white">2.</span> Click the Pull Theory confirmation link.</p><p><span className="font-semibold text-white">3.</span> Return and log in. Your selected membership will continue automatically.</p></div><p className="mt-4 text-xs leading-5 text-zinc-500">Don&apos;t see it? Check your spam or junk folder.</p><a href={loginHref} className="mt-6 block w-full rounded-xl bg-violet-600 px-4 py-3 font-semibold text-white">Continue to login</a></div></div>}
  </main>;
}
