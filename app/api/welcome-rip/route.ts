...
async function prizeForUser(admin: NonNullable<ReturnType<typeof adminClient>>, userId: string) {
  const { data: claim } = await admin
    .from("welcome_rip_claims")
    .select("prize_id,claimed_at")
    .eq("user_id", userId)
    .maybeSingle();
  if (!claim?.prize_id) return null;
  const { data: prize } = await admin
    .from("welcome_rip_prizes")
    .select("id,tier,display_name,value_min_cents,value_max_cents,card_name,card_set,card_number,image_url")
    .eq("id", claim.prize_id)
    .maybeSingle();
...
export async function GET(request: Request) {
  ...
  const [{ data: settings }, prize, inventory] = await Promise.all([
    admin.from("welcome_rip_settings").select("launch_at,active").eq("singleton", true).maybeSingle(),
    prizeForUser(admin, user.id),
    counts(admin),
  ]);
...
export async function POST(request: Request) {
  ...
  if (error) {
    console.error("Welcome Rip claim failed:", error);
    return NextResponse.json({ error: "We couldn't open your Welcome Rip. Please try again." }, { status: 500 });
  }
  ...
  if (!prize) {
    ...
    if (!(user.email_confirmed_at || user.confirmed_at)) return NextResponse.json({ error: "Confirm your email before opening your Welcome Rip." }, { status: 403 });
