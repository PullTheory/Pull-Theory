...
function marketValue(card: CardSuggestion) {
  const value = Object.values(card.tcgplayer?.prices ?? {}).find((price) => typeof price.market === "number")?.market;
...
export default function MarketplaceListingPage() {
  ...
  function chooseCard(card: CardSuggestion) {
    ...
    setCardImage(card.images?.small || ""); setRarity(card.rarity || ""); setSelectedCard(true); setSuggestions([]);
  }
  function startNewSearch(value: string) { setCardName(value); setSelectedCard(false); setCardImage(""); setRarity(""); setDetails(emptyDetails); }
  ...
  async function postListing(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/trades", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ name: username, offeredCard: formatCardTitle(cardName, details), desiredCard: "Open to offers", notes: encodeCardDetails(details), photoUrls, is_listing: true, agree: true }) });
  ...
  return <main className="min-h-screen bg-[#050506]/70 px-4 py-8 text-white sm:px-6 sm:py-12"><div className="mx-auto max-w-3xl">
    ...
    <form onSubmit={postListing} className="mt-6 space-y-6 rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 sm:p-8">
      ...
      <p className="text-xs leading-5 text-zinc-500">Your listing will be open to trade offers. By listing, you agree to send the card to PullShield if you accept an offer. Your shipping information stays private until a trade is accepted.</p>
